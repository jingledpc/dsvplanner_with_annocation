...
namespace dsvplanner_ns {
    OccupancyGrid::OccupancyGrid(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private) : nh_(nh), nh_private_(nh_private) {
        initialize();
    }
    ...
    void OccupancyGrid::terrainCloudAndOdomCallback(const nav_msgs::Odometry::ConstPtr &odom_msg, const sensor_msgs::PointCloud2::ConstPtr &terrain_msg) {
        ...
        for (int i = 0; i < terrainCloudSize; i++) {
            ...
            if (point.intensity > kObstacleHeightThre && point.intensity < kFlyingObstacleHeightThre) {//视为障碍物的点云
                terrain_cloud_obstacle_->push_back(point);
            } else if (point.intensity <= kObstacleHeightThre) {//可通过
    ...
    void OccupancyGrid::updateGrid() {
        ...
        //根据terrain_cloud_traversable_中的点 更新 对应的栅格的状态 
    ...
    std::vector<GridIndex> OccupancyGrid::rayCast(GridIndex origin, GridIndex goal, GridIndex max_grid, GridIndex min_grid) {
        ...
        if (origin == goal) {grid_pairs.push_back(origin);
            return grid_pairs;
        }
