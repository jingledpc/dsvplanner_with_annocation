...
dsvplanner_ns::Drrt::Drrt(volumetric_mapping::OctomapManager *manager, DualStateGraph *graph, DualStateFrontier *frontier, OccupancyGrid *grid) {
    manager_ = manager;
    grid_ = grid;
...
double dsvplanner_ns::Drrt::angleDiff(StateVec direction1, StateVec direction2) {
    double degree;
    degree = acos((direction1[0] * direction2[0] + direction1[1] * direction2[1]) / (sqrt(direction1[0] * direction1[0] + direction1[1] * direction1[1]) *
            sqrt(direction2[0] * direction2[0] + direction2[1] * direction2[1]))) * 180 / M_PI;
    return degree;
}
...
void dsvplanner_ns::Drrt::getNextNodeToClosestGlobalFrontier() {
    ...
    nextNodeFound_ = false;
    for (int i = dual_state_graph_->local_graph_.vertices.size() - 1; i >= 0; i--) // Search from end to the begining.Nodes with large index means they  are closer to the current position of the robot          
    {
        ...
        for (int j = 0; j < dual_state_frontier_->global_frontier_->size(); j++) {
            ...
            if (length1 > (this->manager_->getSensorMaxRange() + params_.kGainRange) || fabs(p1.z() - p2.z()) > params_.kMaxExtensionAlongZ) // Not only
                                                                    // consider the
            // sensor range,
            // also take the
...
void dsvplanner_ns::Drrt::getThreeLocalFrontierPoint() // Three local frontiers that are most close to the last exploration direciton will be selected.
{
    ...
    for (int i = 0; i < localFrontierSize; i++) {
        frontierDirection[0] = dual_state_frontier_->local_frontier_->points[i].x - root_[0]; // For ground robot, we only consider the  direction along x-y plane
...
void dsvplanner_ns::Drrt::plannerIterate() {
    ...
    while (!candidateFound) {
        ...
        if (((double)rand()) / ((double)RAND_MAX) > 0.75 && localThreeFrontier_->size() > 0) {
            ...
            if (!generateNodeArroundFrontierSuccess) { // Generate node near local frontier fail
...
void dsvplanner_ns::Drrt::publishPlanningHorizon() { // Publish visualization of current planning horizon
