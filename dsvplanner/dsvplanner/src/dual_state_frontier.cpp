...
namespace dsvplanner_ns {
    DualStateFrontier::DualStateFrontier(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private, volumetric_mapping::OctomapManager *manager, OccupancyGrid *grid) : nh_(nh), nh_private_(nh_private) {
        manager_ = manager;
        grid_ = grid;
        initialize();
    }
    ...
    bool DualStateFrontier::inSensorRangeofGraphPoints(StateVec point) {
        ...
        if (graphPoints_->points.size() > 0) {
            ...
            for (int i = 0; i < pointSearchInd.size(); i++) {
                ...
                if (manager_->CellStatus::kOccupied != manager_->getVisibility(node_point, point, false) && !grid_->collisionCheckByTerrainWithVector(node_point, point)) {
                    return true;
                }
            }
            return false;
        ...
        for (int i = 0; i < kTerrainVoxelWidth * kTerrainVoxelWidth; i++) {
            if (terrain_voxel_points_num_[i] > 0) {
                if (terrain_voxel_max_elev_[i] - terrain_voxel_min_elev_[i] >= 0.4)
                    terrain_voxel_elev_[i] = 1000; // set a high value to untraversable voxel
                else
