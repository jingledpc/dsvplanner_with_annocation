...
namespace misc_utils_ns{
    ...
    //Utility functions
    template <class FromPointType, class ToPointType> void PointToPoint(const FromPointType & from_point, ToPointType & to_point){
    ...
    */
    ...
    bool LineSegIntersectWithTolerance(const geometry_msgs::Point &p1, const geometry_msgs::Point &q1, const geometry_msgs::Point &p2, const geometry_msgs::Point &q2, const double tolerance);
    ...
    template <class PCLPointType> void LinInterpPoints(const geometry_msgs::Point & p1, const geometry_msgs::Point & p2, double resolution, typename pcl::PointCloud<PCLPointType>::Ptr & cloud){
        double point_dist = PointXYZDist<geometry_msgs::Point, geometry_msgs::Point>(p1, p2);
        if(point_dist < 0.01) return;
        PCLPointType point1;
        point1.x = p1.x;
        point1.y = p1.y;
        point1.z = p1.z;
